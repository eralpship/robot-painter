...
export function extractCanonicalTransform(
  ...
  console.log('[Transform] extractCanonicalTransform called for element:', element.id)
  ...
  if (svgTransform) {
    const translateMatch = svgTransform.match(/translate\(([-\d.]+),\s*([-\d.]+)\)/)
    ...
    if (translateMatch) {
      ...
      console.log('[Transform] Parsed base translate:', { centerX: baseTransform.centerX, centerY: baseTransform.centerY })
    }
    if (rotateMatch) {
    ...
    if (scaleMatch) {
      ...
      console.log('[Transform] Parsed base scale:', { scaleX: baseTransform.scaleX, scaleY: baseTransform.scaleY })
    }
  }
  ...
  for (const component of cssTransform) {
    console.log('[Transform] Component type:', component.constructor.name, component)

    if (component instanceof CSSTranslate) {
      cssTranslateX += component.x.to('px').value
      cssTranslateY += component.y.to('px').value
      console.log('[Transform] CSS translate delta:', { x: component.x.to('px').value, y: component.y.to('px').value })
      ...
      const scaleX = typeof component.x === 'number' ? component.x : (component.x as CSSUnitValue).value
      const scaleY = typeof component.y === 'number' ? component.y : (component.y as CSSUnitValue).value
  ...
  if (Math.abs(result.centerX) > 2000 || Math.abs(result.centerY) > 2000) {
    console.error('[Transform] ASSERTION FAILED: Center values out of expected range!', result)
  }
  ...
  if (result.scaleX < 0 || result.scaleY < 0 || result.scaleX > 10 || result.scaleY > 10) {
    console.error('[Transform] ASSERTION FAILED: Scale values out of expected range!', result)
  }
